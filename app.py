...
# under the License.
#
# Modified by Blake McCulley
# http://mkdir.info
# Code: https://github.com/bmccully/torn-blog
#
...
define("mysql_database", default="tornblog", help="blog database name")
...
class HomeHandler(BaseHandler):
    def get(self):
        ...
        items_per_page = 5
        limit_id = self.db.query("SELECT id FROM entries ORDER BY id "
                                 "DESC LIMIT 1")

        row_count = self.db.query("SELECT count(*) as count FROM entries")
        ...
        entries = self.db.query(
                "SELECT * FROM entries where id < %i ORDER BY id "
                "DESC LIMIT 5" % limit_id)
